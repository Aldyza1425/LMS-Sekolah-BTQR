...
<template>
  <div class="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h2 class="text-2xl font-headline font-extrabold text-[#8B2323] tracking-tight">Tambah Modul</h2>
      ...
      <div class="p-6 md:p-10 space-y-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div class="space-y-2">
            ...
            <input 
              ...
              class="w-full px-6 py-4 rounded-2xl bg-gray-100/50 border-2 border-transparent focus:border-[#8B2323] focus:bg-white transition-all font-extrabold text-gray-900 placeholder:text-gray-400"
            >
          </div>
          <div class="space-y-2">
            ...
            <input 
              ...
              dir="rtl"
              class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none focus:ring-2 focus:ring-[#8B2323] transition-all font-bold text-[#8B2323] text-xl placeholder:text-gray-400"
            >
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div class="space-y-2">
            ...
            <div class="flex gap-2 p-1.5 bg-gray-50 rounded-2xl">
              <button 
                ...
                :class="form.level === lvl ? 'bg-[#8B2323] text-white shadow-lg' : 'text-gray-500 hover:text-gray-700'"
              >
                {{ lvl }}
              </button>
            </div>
          ...
          <div class="space-y-2">
            ...
            <div 
              class="relative group border-2 border-dashed border-gray-100 rounded-[2rem] p-4 hover:border-[#8B2323] transition-all cursor-pointer bg-gray-50/50"
            ...
            >
              ...
              <div v-if="imagePreview" class="relative h-48 w-full rounded-2xl overflow-hidden shadow-md">
              ...
              <div v-else class="h-48 w-full flex flex-col items-center justify-center space-y-4">
                <div class="w-16 h-16 rounded-2xl bg-white shadow-sm flex items-center justify-center text-gray-300 group-hover:text-[#8B2323] transition-colors">
        ...
        <div class="space-y-2">
          ...
          <textarea 
            ...
            class="w-full px-6 py-4 rounded-2xl bg-gray-100/50 border-2 border-transparent focus:border-[#8B2323] focus:bg-white transition-all font-bold text-gray-900 placeholder:text-gray-400 resize-none"
          ></textarea>
        </div>

        <!-- Actions -->
        <div class="pt-6 border-t border-gray-100 flex flex-col sm:flex-row gap-4 justify-end">
          <button @click="router.back()" class="px-10 py-4 rounded-2xl font-bold text-gray-600 hover:bg-gray-50 transition-all uppercase tracking-widest text-xs">
            Batal
          </button>
          <button 
            @click="saveModule"
            :disabled="isLoading"
            class="px-10 py-4 rounded-2xl font-bold bg-[#8B2323] text-white shadow-lg shadow-red-900/20 hover:shadow-xl active:scale-95 transition-all uppercase tracking-widest text-xs disabled:opacity-50"
          >
    ...
    <div class="bg-red-50 p-8 rounded-[2rem] flex gap-6 items-start border border-red-100/50">
      <div class="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-[#8B2323] shadow-sm flex-shrink-0">
        <span class="material-symbols-outlined">lightbulb</span>
      </div>
      <div>
        <h4 class="font-bold text-[#8B2323] mb-1">Tips Pengajar</h4>
        <p class="text-sm text-[#8B2323]/70 leading-relaxed font-medium">Pastikan gambar sampel memiliki resolusi minimal 800x600 piksel untuk tampilan kartu yang optimal di dashboard siswa.</p>
      </div>
    </div>
  </div>
</template>
