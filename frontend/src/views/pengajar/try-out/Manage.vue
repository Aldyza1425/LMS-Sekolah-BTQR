...
<template>
  <div class="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
    ...
    >
      <div v-if="flashMessage.show" class="fixed top-8 right-8 z-[100] w-full max-w-sm overflow-hidden rounded-[2rem] bg-white/80 backdrop-blur-xl border border-white/20 shadow-2xl shadow-red-900/10">
        ...
        <div class="h-1 bg-gray-100 w-full overflow-hidden">
          <div class="h-full transition-all duration-100 ease-linear" :class="flashMessage.type === 'success' ? 'bg-[#8B2323]' : 'bg-red-500'" :style="{ width: flashMessage.progress + '%' }"></div>
        </div>
      </div>
    </transition>
    ...
    <div v-if="deleteConfirmSoal.show" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-gray-900/40 backdrop-blur-sm" @click="closeDeleteConfirmSoal"></div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          <div class="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">
            ...
            <span class="text-[#8B2323]">{{ examData.title }}</span>
          </div>
          <h2 class="text-3xl font-headline font-extrabold text-gray-900 tracking-tight leading-none">Kelola Detail Ujian</h2>
        </div>
      </div>
      <div class="flex gap-3 w-full md:w-auto">
        <button @click="addQuestion" class="flex-1 md:flex-none bg-[#8B2323] text-white px-6 py-3 rounded-xl font-bold text-sm shadow-lg shadow-red-900/10 hover:shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
      ...
      <!-- Nilai Tertinggi -->
      <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-4 min-w-0">
        <div class="w-12 h-12 rounded-2xl flex items-center justify-center bg-yellow-50 text-yellow-600 flex-shrink-0">
      ...
      <!-- Durasi Ujian -->
      <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-4">
        <div class="w-12 h-12 rounded-2xl flex items-center justify-center bg-orange-50 text-orange-600">
    ...
    <div v-if="examData.mulai_at" class="bg-amber-50/70 border border-amber-200/50 p-6 rounded-3xl flex gap-4 items-start animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <!-- Question Bank List -->
    <div class="bg-white rounded-[2.5rem] shadow-2xl shadow-red-900/5 border-t-4 border-[#8B2323] overflow-hidden">
      <div class="p-6 md:p-10">
        <div class="flex items-center justify-between mb-8 px-2 border-l-4 border-[#8B2323] pl-4">
           <h3 class="text-xl font-headline font-bold text-gray-900">Bank Soal Ujian</h3>
           <span class="text-[10px] font-extrabold px-3 py-1 bg-gray-50 text-gray-400 rounded-full border border-gray-100 uppercase tracking-widest">{{ examData.questions.length }} Total Soal</span>
        </div>
        
        <div class="space-y-4">
          <div v-for="(q, i) in examData.questions" :key="q.id" class="group bg-gray-50/50 p-6 rounded-3xl border border-gray-100 hover:bg-gray-100/50 hover:border-gray-200 transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div class="flex items-start gap-4 flex-1">
              <div class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center font-bold text-[#8B2323] flex-shrink-0">
                {{ i + 1 }}
              </div>
              <div class="space-y-2">
                <p class="font-bold text-gray-900 group-hover:text-[#8B2323] transition-colors leading-relaxed">{{ q.pertanyaan }}</p>
                <div v-if="q.gambar" class="mt-2">
                  <img :src="q.gambar" alt="Gambar Soal" class="h-16 rounded-xl border border-gray-100 object-contain" />
                </div>
                <div class="flex items-center gap-3">
                   <span class="text-[9px] font-bold uppercase tracking-widest text-gray-400">{{ q.tipe === 'pilihan_ganda' ? 'Pilihan Ganda' : 'Essay' }}</span>
                   <span class="w-1 h-1 bg-gray-200 rounded-full"></span>
                   <span class="text-[9px] font-bold uppercase tracking-widest text-gray-500">Kunci: {{ q.jawaban.toUpperCase() }}</span>
                   <span class="w-1 h-1 bg-gray-200 rounded-full"></span>
                   <span class="text-[9px] font-bold uppercase tracking-widest text-blue-500">Bobot: {{ q.bobot }}</span>
                </div>
              </div>

            </div>
            <div class="flex gap-2 w-full sm:w-auto">
              <button @click="editQuestion(q.id)" class="flex-1 sm:flex-none p-3 rounded-xl bg-white text-gray-400 hover:bg-[#8B2323] hover:text-white transition-all shadow-sm">
                <span class="material-symbols-outlined text-lg">edit</span>
              </button>
              <button @click="deleteQuestion(q.id)" class="flex-1 sm:flex-none p-3 rounded-xl bg-white text-gray-400 hover:bg-black hover:text-white transition-all shadow-sm">
                <span class="material-symbols-outlined text-lg">delete</span>
              </button>
            </div>
          </div>
        ...
        <div 
          @click="addQuestion"
          class="mt-8 border-2 border-dashed border-gray-200 p-8 rounded-[2rem] flex flex-col items-center justify-center text-center cursor-pointer hover:border-[#8B2323]/50 hover:bg-red-50/10 transition-all group"
        >
          <div class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center text-gray-300 group-hover:text-[#8B2323] transition-colors mb-4">
            <span class="material-symbols-outlined text-4xl">add_task</span>
          </div>
          <p class="font-bold text-gray-500 group-hover:text-[#8B2323] uppercase tracking-[0.2em] text-xs">Tambahkan Butir Soal Baru</p>
        </div>
      </div>
    </div>
  </div>
