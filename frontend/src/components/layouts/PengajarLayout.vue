...
<template>
  <div class="min-h-screen bg-white font-['Manrope'] overflow-x-hidden">
    ...
    <!-- SideNavBar -->
    <aside 
      class="bg-[#8B2323] h-screen w-60 fixed left-0 top-0 flex flex-col py-6 px-5 z-50 overflow-y-auto font-semibold shadow-[0_12px_32px_-4px_rgba(139,35,35,0.08)] transition-transform duration-300 md:translate-x-0"
    ...
    >
      <div class="mb-10 px-4 flex justify-between items-center">
        <div>
          ...
          <p class="text-white/60 text-[10px] font-normal mt-0.5 uppercase tracking-widest">Pengajar Portal</p>
        </div>
      ...
      <nav class="flex-1 space-y-2">
        ...
        <RouterLink 
          ...
          exact-active-class="bg-white text-[#8B2323] md:-mr-4 shadow-sm"
      ...
      <div class="mt-auto pt-6 border-t border-white/10 space-y-1 text-white">
        <!-- Profile -->
        <RouterLink 
          to="/lms/pengajar/profile" 
          class="flex items-center gap-3 px-4 py-2.5 rounded-full text-sm transition-all"
          :class="route.path.startsWith('/lms/pengajar/profile') ? 'bg-white/15 text-white' : 'text-white/80 hover:bg-white/10 hover:text-white'"
        >
    ...
    <main class="min-h-screen bg-white transition-all duration-300 md:ml-60">
      ...
      <header class="flex justify-between items-center w-full px-4 md:px-8 py-3 bg-white sticky top-0 z-40 border-b border-gray-100 md:border-none">
        <div class="flex items-center gap-3 md:gap-4 text-outline">
          <button @click="toggleSidebar" class="p-2 -ml-2 text-[#8B2323] md:hidden">
            <span class="material-symbols-outlined">menu</span>
          </button>
          <span class="text-xs font-bold uppercase tracking-widest opacity-40 hidden sm:inline">Pengajar</span>
          <span class="material-symbols-outlined text-xs hidden sm:inline">chevron_right</span>
          <span class="text-[#8B2323] font-bold border-b-2 border-[#8B2323] capitalize text-sm">{{ route.path.split('/').pop() || 'Dashboard' }}</span>
        </div>
        <div class="flex items-center gap-4 md:gap-6">
          <NotificationBell 
            themeColor="#8B2323" 
            hoverColorClass="hover:text-[#8B2323]" 
          ...
          <div class="flex items-center gap-3 pl-4 border-l border-gray-200">
            ...
            <div class="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center text-[#8B2323] shadow-sm overflow-hidden">
